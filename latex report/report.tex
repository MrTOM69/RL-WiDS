...
% Removing the default chapter starting from new page
\usepackage{etoolbox}
\makeatletter
...
\chapter{Financial Markets Background}
...
\section{Challenges of Algorithmic Trading}
...
At the core of reinforcement learning lies the Markov Decision Process framework. An MDP is formally defined by a tuple $(\mathcal{S}, \mathcal{A}, P, R, \gamma)$, where $\mathcal{S}$ denotes the state space, $\mathcal{A}$ the action space, $P(s'|s,a)$ the state transition probabilities, $R(s,a)$ the reward function, and $\gamma \in [0,1)$ the discount factor.
...
\section{Policy-Based vs.\ Value-Based Methods}
...
\chapter{Dataset and Market Data}
...
\chapter{Trading Environment Design}
...
\chapter{Reward Function Design}
...
\section{Risk-Aware Considerations}
...
\chapter{Model Architecture and Algorithms}
...
Training proceeds by iterating over multiple episodes of historical data. During training, the agent employs an $\epsilon$-greedy exploration strategy, selecting a random action with probability $\epsilon$ and the greedy action otherwise. This mechanism balances exploration of new actions with exploitation of learned value estimates.
...
\chapter{Limitations and Failure Modes}
...
\chapter{Ethical and Financial Risks}
